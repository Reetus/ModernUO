  ...
  public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
  {
    ...
    public virtual TimeSpan OnSwing(Mobile attacker, Mobile defender, double damageBonus)
    {
      ...
      if (Core.AOS)
      {
        ...
        if (canSwing) canSwing = !(attacker is PlayerMobile p) || p.PeacedUntil <= DateTime.UtcNow;
      }
        ...
        if (blocked)
        {
          ...
          if (Confidence.IsConfident(defender))
          {
            defender.SendLocalizedMessage(
              1063117); // Your confidence reassures you as you successfully block your opponent's blow.
    ...
    public virtual void OnHit(Mobile attacker, Mobile defender, double damageBonus = 1.0)
    {
      ...
      if (a != null) percentageBonus += (int)(a.DamageScalar * 100) - 100;

      if (move != null) percentageBonus += (int)(move.GetDamageScalar(attacker, defender) * 100) - 100;
      ...
      if (packInstinctBonus != 0) percentageBonus += packInstinctBonus;

      if (InDoubleStrike) percentageBonus -= 10;
      ...
      if (Core.AOS)
      {
        int physChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitPhysicalArea) *
                               propertyBonus);
        int fireChance =
        ...
        int poisChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitPoisonArea) *
                               propertyBonus);
        int nrgyChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitEnergyArea) *
                               propertyBonus);
        ...
        int laChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitLowerAttack) *
                             propertyBonus);
        int ldChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitLowerDefend) *
                             propertyBonus);
    ...
    public virtual void OnMiss(Mobile attacker, Mobile defender)
    {
      ...
      if (defender is IHonorTarget target) target.ReceivedHonorContext?.OnTargetMissed(attacker);
    }
    ...
    public int GetElementalDamageHue()
    {
      ...
      if (cold >= currentMax) hue = 1261 + (cold - 50) / 10;
    ...
    public virtual void DoDispel(Mobile attacker, Mobile defender)
    {
      ...
      {
        Effects.SendLocationParticles(EffectItem.Create(defender.Location, defender.Map, EffectItem.DefaultDuration),
          0x3728, 8, 20, 5042);
    ...
    public virtual void DoAreaAttack(Mobile from, Mobile defender, int sound, int hue, int phys, int fire, int cold,
      int pois, int nrgy)
    {
      Map map = from.Map;
      ...
      for (int i = 0; i < list.Count; ++i)
      {
        ...
        if (scalar < 1.0) damage *= (11 - from.GetDistanceToSqrt(m)) / 10;
